# code taken from: https://github.com/modelscope/modelscope-agent/blob/master/modelscope_agent/utils/logger.py
import json
import logging
import os
...
class AgentLogger:
    ...
    def set_file_handle(self, log_dir, max_bytes=50 * 1024 * 1024, backup_count=7):
        """
        Args:
        ...
        file_log_formatter = get_formatter(os.getenv(LOG_FILE_FORMAT, "json").lower())
        info_file_path = os.path.join(log_dir, INFO_LOG_FILE_NAME)
        info_file_handler = RotatingFileHandler(
            info_file_path, mode="a", maxBytes=max_bytes, backupCount=backup_count
        )
        ...
        error_file_handler = RotatingFileHandler(
            error_file_path, mode="a", maxBytes=max_bytes, backupCount=backup_count
        )
